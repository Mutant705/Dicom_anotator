...
class DICOMVisualizer:
    def __init__(self, frame_obj):
        ...
        # Load raw data to GPU (float32 for better math precision)
        ...
        # --- NORMALIZATION STATE ---
        self.norm_mode = "Linear"  # Modes: Linear, Skewed, Z-Score, Log
        self.__params = {
    ...
    def __setup_ui(self):
        ...
        # --- NORMALIZATION MODE BUTTONS (RIGHT SIDE) ---
        self.__norm_btns = {}
        ...
        for name, pos in modes:
            ...
            btn.on_clicked(lambda e, n=name: self.__set_norm_mode(n))
            self.__norm_btns[name] = btn

        # --- DYNAMIC TEXT BOXES (BOTTOM) ---
        self.__ax_v1 = self.__fig.add_axes([0.3, 0.08, 0.12, 0.03])
        self.__ax_v2 = self.__fig.add_axes([0.55, 0.08, 0.12, 0.03])
        self.__txt_v1 = TextBox(self.__ax_v1, "v1", initial="0")
        self.__txt_v2 = TextBox(self.__ax_v2, "v2", initial="0")
        self.__txt_v1.on_submit(self.__update_params)
        self.__txt_v2.on_submit(self.__update_params)
    ...
    def __set_norm_mode(self, mode):
        self.norm_mode = mode
        self.__update_norm_ui()
        self.__render_stencil()
        self.__refresh_view()

    def __update_norm_ui(self):
        # Highlight active mode button
        for name, btn in self.__norm_btns.items():
            btn.label.set_color('red' if name == self.norm_mode else 'black')
        
        # Update text box labels and values based on current mode
